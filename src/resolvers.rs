...
//! - signer selectors may load OWS wallets or explicit local signer inputs,
//! - public user and acting-account selectors may resolve OWS wallet names/ids,
...
fn resolve_account_selector_address(selector: &str) -> Result<Address, anyhow::Error> {
    ...
    Err(auth::account_selector_not_found(trimmed).into())
}
