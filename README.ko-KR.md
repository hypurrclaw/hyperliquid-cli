...
- **에이전트를 위한 지갑.** 거래는 가능하지만 출금은 절대 할 수 없는 API wallet(일명 agent wallet)을 생성하세요. 이를 OpenClaw, Hermes, Claude 또는 어떤 자동화에도 전달하면 제한된 권한 안에서 동작합니다. 지갑은 암호화된 OWS vault에 저장되며, 비밀은 stdout, 로그, 셸 기록에 절대 노출되지 않습니다.
...
- **기본적으로 안전함.** mainnet 변경 작업은 확인 프롬프트로 보호됩니다. `--dry-run`은 모든 부작용을 발생 전에 미리 보여줍니다. Testnet은 플래그 하나면 됩니다. API wallets는 프로토콜 설계상 출금이 불가능합니다.
- **Decimal 정확성.** 모든 가격, 크기, 금액은 `rust_decimal`을 사용합니다. float도, 예상치 못한 반올림도 없습니다.
- **단일 정적 바이너리.** [`hypersdk`](https://github.com/infinitefield/hypersdk) 위에서 Rust로 구축되었습니다. 몇 초 만에 설치하고, 컨테이너에 담아 배포하고, 어디서든 실행하세요.
...
설치 프로그램은 바이너리를 `~/.local/bin`에 복사하기 전에 SHA-256 체크섬을 검증합니다. `HYPERLIQUID_CLI_REPO=OWNER/REPO`, `HYPERLIQUID_CLI_VERSION=v0.1.0`, `BIN_DIR=/path/to/bin`으로 기본값을 재정의할 수 있습니다.
...
`hyperliquid`는 **Open Wallet Standard (OWS)**를 유일한 지갑 백엔드로 사용합니다. 지갑은 디스크의 암호화된 vault(기본값 `~/.hyperliquid`, `HYPERLIQUID_OWS_VAULT_PATH`로 재정의 가능)에 저장됩니다. 비밀은 숨김 프롬프트에서 대화식으로 입력되며, 절대 에코, 로깅 또는 출력되지 않습니다.
...
| Domain | Examples |
| --- | --- |
| Local signing account | `account add`, `account ls`, `account set-default` 및 관련 명령으로 관리되는 OWS wallet입니다. |
...
| `USER` | `0x` user address, 또는 문서화된 안전한 stored-account selector | `account portfolio`, `orders status --user`, fee queries 같은 공개 조회입니다. 저장된 API/agent wallet selectors는 이러한 읽기에서 승인한 master account로 해석됩니다. |
...
| Option | Description |
| --- | --- |
| `-f, --format pretty\|table\|json` | 출력 형식입니다. 기본값은 `pretty`입니다. |
...
| `book <COIN_OR_PAIR> [-w]` | L2 order book snapshot을 표시하거나 updates를 watch합니다. |
| `mids [-w]` | 모든 mid prices를 표시합니다. |
| `candles <COIN> [--interval <INTERVAL>] [--limit <N>] [-w]` | candle history를 표시합니다. |
...
| `wallet import-mnemonic [MNEMONIC]` | BIP-39 mnemonic phrase에서 wallet을 가져옵니다. |
...
| `wallet export <SELECTOR>` | wallet secret(mnemonic 또는 private key)을 내보냅니다. |
| `wallet reset` | 확인 후 wallet configuration을 제거합니다. |
| `account fees <ADDRESS>` | fee schedule과 volume context를 조회합니다. |
| `account fills <ADDRESS> [--start <TIME>] [--end <TIME>] [--aggregate-by-time]` | 공개 fill history를 조회하며, 선택적으로 시간별 집계가 가능합니다. |
| `account ledger <ADDRESS> --start <TIME> [--end <TIME>]` | deposits, withdrawals, transfers 및 기타 non-funding ledger updates를 조회합니다. |
| `account funding <ADDRESS> --start <TIME> [--end <TIME>]` | user funding payment history를 조회합니다. |
| `account orders <ADDRESS>` | 공개 open orders를 조회합니다. |
| `account portfolio <ADDRESS>` | 공개 portfolio summary를 조회합니다. |
| `account portfolio-history <ADDRESS>` | frontend portfolio graph/history data를 조회합니다. |
| `account rate-limit <ADDRESS>` | user rate-limit context를 조회합니다. |
| `account subaccounts <ADDRESS>` | 공개 subaccounts를 조회합니다. |
| `account twap-history <ADDRESS>` | user TWAP order history를 조회합니다. |
| `account twap-fills <ADDRESS> [--start <TIME>] [--end <TIME>]` | user TWAP slice fills를 조회합니다. |
...
| `subaccount list <ADDRESS>` | master address의 공개 subaccounts를 조회합니다. |
| `subaccount create --name <NAME>` | master account가 서명한 subaccount를 생성합니다. |
| `account add` / `account ls` / `account set-default` / `account remove` | 저장된 wallets를 관리합니다. |
| `api-wallet create --name <NAME> [--expires-in <DURATION>]` | Hyperliquid API/agent wallet을 생성하고 승인합니다. |
| `api-wallet approve --agent-address <ADDRESS>` | 기존 또는 생성된 agent wallet address를 승인합니다. |
...
API wallets는 승인한 master account를 위해 trading actions에 서명할 수 있지만 출금은 할 수 없습니다. 정보 쿼리에는 master 또는 subaccount address를 사용하세요. CLI는 생성된 API wallets를 master address와 함께 `agent-wallet` records로 저장하므로 저장된 agent reads는 master account로 해석됩니다. `api-wallet create`가 local agent keypair를 생성하면, 해당 주소에 대해 `approveAgent`를 제출하기 전에 private key를 한 번 출력합니다.

### 거래와 이체

| Command | Description |
| --- | --- |
| `orders open [-w]` | open orders를 나열합니다. |
| `orders history` | order history를 나열합니다. |
| `orders status --user <ADDRESS> --oid <OID>` | 공개 order status를 조회합니다. |
| `orders create --coin <COIN> --side buy\|sell ... [--reduce-only] [--on-behalf-of <ACCOUNT_SELECTOR>]` | limit, market, stop-loss, take-profit, stop-limit, take-limit orders를 생성합니다. `--on-behalf-of`는 `vaultAddress`로 사용되는 acting-account selector입니다. |
...
| `orders tpsl --coin <COIN> --take-profit <PX> [--stop-loss <PX>] --grouping position-tpsl` | 현재 position에 연결된 TP/SL orders를 생성합니다. |
| `orders cancel <OID>` / `orders cancel --cloid <CLOID>` | order ID 또는 client order ID로 취소합니다. |
| `orders cancel-all [--coin <COIN>] [-y]` | 모든 open orders를 취소하며, 선택적으로 coin으로 필터링합니다. |
| `orders modify <OID> [--price <PRICE>] [--size <SIZE>]` | 기존 order를 수정합니다. |
| `orders twap-create --coin <COIN> --side buy\|sell --size <SIZE> --duration <SECONDS>` | TWAP order를 생성합니다. |
| `orders twap-cancel <TWAP_ID> --coin <COIN>` | TWAP order를 취소합니다. |
| `orders schedule-cancel --in <DURATION>` | dead man's switch를 설정합니다. |
| `positions list [-w]` | open positions를 나열합니다. |
| `positions update-leverage --coin <COIN> --leverage <N>` | leverage를 업데이트합니다. |
| `positions update-margin --coin <COIN> --amount <AMOUNT>` | isolated margin을 추가하거나 제거합니다. |
| `transfer spot-to-perp --amount <USDC>` | USDC를 spot에서 perp로 이동합니다. |
| `transfer perp-to-spot --amount <USDC>` | USDC를 perp에서 spot으로 이동합니다. |
| `transfer send --to <ADDRESS> --amount <USDC>` | USDC를 다른 address로 보냅니다. |
| `transfer spot-send --to <ADDRESS> --token <TOKEN> --amount <AMOUNT>` | spot token을 다른 address로 보냅니다. |
| `transfer send-asset --to <ADDRESS> --source perp\|spot\|dex:<DEX> --dest perp\|spot\|dex:<DEX> --token <TOKEN> --amount <AMOUNT>` | asset을 accounts, spot, perp, 또는 DEX contexts 사이에서 보냅니다. |
| `transfer withdraw --to <ADDRESS> --amount <USDC>` | USDC를 Arbitrum으로 출금합니다. |
| `subaccount transfer --subaccount <ACCOUNT_SELECTOR> --amount <USDC> --direction deposit\|withdraw` | USDC를 subaccount로 또는 subaccount에서 이동합니다. subaccount field는 generic transfer recipient가 아니라 acting-account selector입니다. |
| `subaccount spot-transfer --subaccount <ACCOUNT_SELECTOR> --token <TOKEN> --amount <AMOUNT> --direction deposit\|withdraw` | spot token을 subaccount로 또는 subaccount에서 이동합니다. subaccount field는 generic transfer recipient가 아니라 acting-account selector입니다. |
...
| `staking delegate` / `staking undelegate` / `staking deposit` / `staking withdraw` / `staking claim-rewards` | staking actions를 제출합니다. |
...
| `vault list [--kind protocol|user|normal|child|parent] [--user <ADDRESS>]` / `vault search <QUERY> [--user <ADDRESS>]` / `vault get <ADDRESS>` / `vault positions <ADDRESS>` | vault state를 발견하고 조회합니다. `--user`는 API가 반환할 때 user deposit context를 포함합니다. |
| `vault deposit` / `vault withdraw` | vault transfers를 제출합니다. |
...
| `builder approve --builder <ADDRESS> --max-fee-rate <PERCENT>` | 설정된 master signer에 대해 builder fee cap을 승인하거나 철회합니다. |
| `prio status` / `prio bid` | gossip priority auction을 조회하거나 입찰합니다. |
| `referral register <CODE>` / `referral set [CODE]` / `referral status` | 자신의 referral code를 등록하거나, referrer를 설정하거나, referral state를 검사합니다. |
| `feedback --scenario-json <JSON>` / `feedback --scenario-file <PATH\|->` | scenario JSON object 형태의 구조화된 CLI feedback을 설정된 feedback endpoint로 보냅니다. rate-limit attribution을 위해 scenario에 `agent_address`, `signer_address`, 또는 `wallet_address`를 포함하고, 기본값을 재정의하려면 `--url`을 사용하세요. |
| `schema [COMMAND...]` | 에이전트를 위한 machine-readable command schemas를 표시합니다. |
| `subscribe trades\|orderbook\|candles\|all-mids\|order-updates\|fills` | WebSocket events를 스트리밍합니다. |
...
- 프롬프트로 보호된 live mainnet mutations와 파괴적인 local secret operations는 지원되는 경우 `-y` / `--yes`가 제공되지 않는 한 confirmation이 필요합니다.
