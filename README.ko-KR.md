...
설치 프로그램은 바이너리를 `~/.local/bin`에 복사하기 전에 SHA-256 체크섬을 검증합니다. 기본적으로 최신 release를 설치합니다. `HYPERLIQUID_CLI_REPO=OWNER/REPO`, `HYPERLIQUID_CLI_VERSION=v0.1.0`, `BIN_DIR=/path/to/bin`으로 repo, 고정 version, install directory를 재정의할 수 있습니다.
...
| `update` | 최신 GitHub release에서 이 바이너리를 업데이트합니다. 전역 `--dry-run`으로 preview할 수 있습니다. |
