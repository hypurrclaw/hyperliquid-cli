...
check_file_contains .github/workflows/release.yml 'x86_64-pc-windows-msvc' "release workflow must package Windows x86_64"
check_file_contains .github/workflows/release.yml 'dist/\$asset' "release workflow must package Windows zip assets"
check_file_contains install.sh 'asset="\$\{BINARY_NAME\}-\$\{target\}\.\$\{archive_ext\}"' "install.sh asset name must match release workflow archive naming"
check_file_contains install.sh 'pc-windows-msvc' "install.sh must support the Windows release target"
