...
| `orders tpsl --coin <COIN> (--take-profit <PX\|+N%\|entry>\|--stop-loss <PX\|-N%\|entry>) [--grouping position-tpsl] [--side buy\|sell] [--size <SIZE>] [--margin-mode cross\|isolated] [--on-behalf-of <ACCOUNT_SELECTOR>]` | Create TP/SL orders attached to the current position. Percent and `entry` resolve against the position entry. `--on-behalf-of` targets the acting subaccount/vault position. |
...
| `orders bracket --coin <COIN> --side buy\|sell --take-profit <PX\|+N%> --stop-loss <PX\|-N%> [--size <SIZE>\|--amount <USDC>] [--entry market\|limit] [-y]` | Place entry then arm TP/SL. Limit entries that do not fill return `entry_resting_unprotected` (exit 15). |
